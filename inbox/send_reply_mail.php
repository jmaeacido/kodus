...
$stmt->execute();
$replyRow = db_stmt_fetch_one_assoc($stmt);
$stmt->close();
...
");
if ($recipientStmt) {
    $messageId = (int) ($replyRow['message_id'] ?? 0);
    $recipientStmt->bind_param('i', $messageId);
    $recipientStmt->execute();
    $recipientRows = db_stmt_fetch_all_assoc($recipientStmt);
    $recipientStmt->close();
    $threadRecipients = array_values(array_filter(array_map(
        static fn(array $recipientRow): string => trim((string) ($recipientRow['recipient_email'] ?? '')),
        $recipientRows
    )));
}
