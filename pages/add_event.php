...
try {
    ...
    if ($id === '') {
        ...
        $stmt = $conn->prepare("
            INSERT INTO events 
            (title, description, all_day, start, end, location, color, is_private, created_by) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->bind_param(
            "ssissssii", 
        ...
        if ($stmt->execute()) {
            ...
            calendarSyncEventGuests($conn, $newEventId, $guestEmails);
        ...
        $eventId = (int) $id;
        $ownerStmt = $conn->prepare("SELECT created_by FROM events WHERE id = ? LIMIT 1");
        if (!$ownerStmt) {
            throw new RuntimeException('Unable to verify event ownership.');
        }
        $ownerStmt->bind_param("i", $eventId);
        $ownerStmt->execute();
        $existingEvent = db_stmt_fetch_one_assoc($ownerStmt);
        $ownerStmt->close();
        ...
        if ($userType !== 'admin' && (int) ($existingEvent['created_by'] ?? 0) !== $userId) {
            http_response_code(403);
        ...
        if ($stmt->execute()) {
            calendarSyncEventGuests($conn, $eventId, $guestEmails);
            if (!$all_day) {
                calendarReplaceEventSchedules($conn, $eventId, $timedSchedules);
            }
