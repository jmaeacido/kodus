...
<head>
  ...
  <style>
    ...
    .calendar-tools .card-header,
    .calendar-tools .card-body {
      padding: 0.85rem 0.95rem;
    }
    .calendar-tools .sticky-top {
      top: 4.8rem;
    }
    ...
    @media (max-width: 1600px) {
      .calendar-tools .small-box .inner {
        padding: 0.9rem;
      }
      .calendar-tools .agenda-item {
        padding: 0.68rem;
...
<div class="wrapper">
                ...
                <div class="card-body">
                  <div id="external-events">
                    <?php
                      ...
                      $res->execute();
                      $result = db_stmt_fetch_all_assoc($res);
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <p id="viewEventCreatedByRow" style="display:none;"><strong>Created by:</strong><br> <span id="viewEventCreatedBy"></span></p>
          <p id="viewEventUpdatedByRow" style="display:none;"><strong>Updated by:</strong><br> <span id="viewEventUpdatedBy"></span></p>
  ...
  <div class="modal fade" id="eventModal" tabindex="-1">
    <div class="modal-dialog">
      <form id="eventForm" class="modal-content" data-no-loader="true">
        <div class="modal-header">
...
<script src="<?php echo isset($app_root) ? $app_root . 'dist/js/adminlte.min.js' : '../dist/js/adminlte.min.js'; ?>"></script>
...
$(function () {
  ...
  function syncCreatedByDisplay(ev) {
    const createdBy = String(ev?.extendedProps?.createdBy || '').trim();
    const isPublicEvent = !ev?.extendedProps?.isPrivate;

    if (isPublicEvent && createdBy !== '') {
      $('#viewEventCreatedBy').text(createdBy);
      $('#viewEventCreatedByRow').show();
      $('#createdByWrapper').show();
      $('#eventModal').find('label.createdByLabel').text("Created by: " + createdBy);
      return;
    }

    $('#viewEventCreatedBy').text('');
    $('#viewEventCreatedByRow').hide();
    $('#createdByWrapper').hide();
    $('#eventModal').find('label.createdByLabel').text("Created by:");
  }

  function syncUpdatedByDisplay(ev) {
    const updatedBy = String(ev?.extendedProps?.updatedBy || '').trim();
    const isPublicEvent = !ev?.extendedProps?.isPrivate;

    if (isPublicEvent && updatedBy !== '') {
      $('#viewEventUpdatedBy').text(updatedBy);
      $('#viewEventUpdatedByRow').show();
      $('#updatedByWrapper').show();
      $('#eventModal').find('label.updatedByLabel').text("Updated by: " + updatedBy);
      return;
    }

    $('#viewEventUpdatedBy').text('');
    $('#viewEventUpdatedByRow').hide();
  ...
  function openEventDetails(ev) {
    ...
    syncCreatedByDisplay(ev);
    syncUpdatedByDisplay(ev);
  ...
  function withCsrf(payload) {
    let data = payload && typeof payload === 'object' ? Object.assign({}, payload) : {};
    if (!Object.prototype.hasOwnProperty.call(data, 'csrf_token')) {
      data.csrf_token = window.KODUS_CSRF_TOKEN || '';
    }
    return data;
  }
  ...
  function showCalendarSuccessAlert(title, callback) {
    const runAlert = function() {
      Swal.fire({
        icon: 'success',
        title: title || 'Saved!',
        timer: 1500,
    ...
    if (modal.hasClass('show')) {
      ...
      if (activeElement && modal[0] && modal[0].contains(activeElement) && typeof activeElement.blur === 'function') {
        activeElement.blur();
      }

      modal.one('hidden.bs.modal', runAlert);
      modal.modal('hide');
      return;
    }
  ...
  function closeModalThenRun(selector, callback) {
    ...
    const activeElement = document.activeElement;
    if (activeElement && modal[0].contains(activeElement) && typeof activeElement.blur === 'function') {
      activeElement.blur();
    }
  ...
  function confirmDeleteEvent(id, modalSelector) {
    ...
    closeModalThenRun(modalSelector, function() {
      Swal.fire({
        title: 'Delete this event?',
        text: 'This event will be removed from your calendar.',
        icon: 'warning',
        ...
        $.post('delete_event.php', withCsrf({id: id}), function(res) {
          const data = parseAjaxJsonResponse(res);
          if (data.status === 'deleted' || data.success) {
            showCalendarSuccessAlert('Deleted!', function() {
              calendar.refetchEvents();
            });
            return;
          }

          Swal.fire('Error', data.message || data.error || 'Delete failed.', 'error');
        }).fail(function(xhr) {
          Swal.fire('Error', xhr.responseJSON?.message || xhr.responseJSON?.error || 'Delete failed.', 'error');
        });
      });
    });
  }
  ...
  let calendar = new FullCalendar.Calendar(document.getElementById('calendar'), {
    ...
    eventReceive: function(info) {
      ...
      // Save to DB
      $.post('add_event.php', withCsrf(newEvent), function(res) {
        try {
    ...
    eventDragStop: function(info) {
      ...
      if (x >= trash.left && x <= trash.right && y >= trash.top && y <= trash.bottom) {
        ...
        if (info.event.extendedProps && info.event.extendedProps.dbId) {
          ...
          $.post('delete_draggable.php', withCsrf({id: info.event.extendedProps.dbId}), function() {
            info.event.remove();
            Swal.fire('Deleted!', '', 'success');
          });
        } else {
          // normal calendar event
          $.post('delete_event.php', withCsrf({id: info.event.id}), function() {
            info.event.remove();
            Swal.fire('Deleted!', '', 'success');
            calendar.refetchEvents();
          });
  ...
  $('#viewDuplicateBtn').click(function() {
    ...
    closeModalThenRun('#viewEventModal', function() {
      $('#eventForm')[0].reset();
      $('#eventId').val('');
  ...
  $('#viewDeleteBtn').click(function() {
    let id = $(this).data('id');
    confirmDeleteEvent(id, '#viewEventModal');
  });
  ...
  function saveEvent(event) {
    ...
    $.post('update_event.php', withCsrf(payload), function(res) {
      try {
        let data = parseAjaxJsonResponse(res);
        if (data.success) {
  ...
  $('#eventForm').submit(function(e){
    ...
    $.post(url, withCsrf(payload), function(res){
      try {
        let data = parseAjaxJsonResponse(res);
        if (data.success || data.id) {
          showCalendarSuccessAlert('Saved!', function() {
            calendar.refetchEvents();
          });
        } else Swal.fire('Error', data.message || 'Save failed','error');
      } catch(e){ console.error(e); Swal.fire('Error','Invalid server response','error'); }
    }).fail(function(xhr){
      Swal.fire('Error', xhr.responseJSON?.message || xhr.responseJSON?.error || 'Cannot save event', 'error');
    });
  });

  $('#deleteEvent').click(function() {
    let id = $('#eventId').val();
    confirmDeleteEvent(id, '#eventModal');
  });
  ...
  $('#add-new-event').click(function(e) {
    ...
    if (!val) return;
    $.post('add_draggable.php', withCsrf({title: val, color: currColor}), function(res) {
      let data = typeof res === 'object' ? res : JSON.parse(res);
      if (!data.id) {
