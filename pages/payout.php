...
let dailyWageRate = <?php echo json_encode($dailyWageRate); ?>;
let payoutDays = <?php echo json_encode($payoutDays); ?>;
let beneficiaryPayoutRate = <?php echo json_encode($beneficiaryPayoutRate); ?>;
...
<body>
<div class="wrapper"><div class="content-wrapper"><div class="content"><div class="container-fluid"><div class="row"><div class="col-lg-12"><div class="card card-primary card-outline"><div class="card-header d-flex align-items-center"><h5 class="m-0 flex-grow-1">Payout Details by Municipality</h5><button id="exportBtn" class="btn btn-info btn-sm">Export to Excel</button></div><div class="card-body" id="payout-card-body">
<div id="payout-runtime-config" data-daily-wage-rate="<?= htmlspecialchars((string) $dailyWageRate, ENT_QUOTES, 'UTF-8') ?>" data-payout-days="<?= htmlspecialchars((string) $payoutDays, ENT_QUOTES, 'UTF-8') ?>" data-beneficiary-payout-rate="<?= htmlspecialchars((string) $beneficiaryPayoutRate, ENT_QUOTES, 'UTF-8') ?>" hidden></div>
<?php if ($dailyWageRate <= 0): ?>
<div class="alert alert-warning">No project variable is configured yet for <strong>daily_wage_rate</strong> in fiscal year <?= htmlspecialchars((string) $year, ENT_QUOTES, 'UTF-8') ?>. Ask an administrator or editor to update <a href="<?= htmlspecialchars($app_root, ENT_QUOTES, 'UTF-8') ?>admin/project_variables">Project Variables</a>.</div>
<?php endif; ?>
<?php if ($payoutDays <= 0): ?>
<div class="alert alert-warning">No project variable is configured yet for <strong>working_days</strong> in fiscal year <?= htmlspecialchars((string) $year, ENT_QUOTES, 'UTF-8') ?>. Ask an administrator or editor to update <a href="<?= htmlspecialchars($app_root, ENT_QUOTES, 'UTF-8') ?>admin/project_variables">Project Variables</a>.</div>
<?php endif; ?>
<div class="table-container kodus-table-scroll" style="--kodus-table-min-width: 1200px;"><table id="sectoralTable" class="table table-bordered table-striped" style="text-align:center; width:100%;"><thead><tr><th>Action</th><th>Province</th><th>City or Municipality</th><th>Barangays</th><th>No. of Partner-Beneficiaries</th><th>Amount</th><th>Payout Date</th><th>Paid</th><th>Amount Paid</th><th>Unpaid</th><th>Amount Unpaid</th></tr><tr style="font-weight:bold;"><td colspan="4">Total</td><td><?= number_format($totalBenes) ?></td><td><?= number_format($totalAmount, 2) ?></td><td></td><td><?= number_format($totalPaid) ?></td><td><?= number_format($totalAmountPaid, 2) ?></td><td><?= number_format($totalUnpaid) ?></td><td><?= number_format($totalAmountUnpaid, 2) ?></td></tr></thead><tbody style="font-size:12px;">
...
function initializePayoutTable() {
    if ($.fn.dataTable.isDataTable('#sectoralTable')) {
        $('#sectoralTable').DataTable().destroy();
    }

    $('#sectoralTable').DataTable({ responsive: false, scrollX: true, autoWidth: false, ordering: false, lengthMenu: [[10, 25, 50, 100, 200, -1], [10, 25, 50, 100, 200, 'All']] });
}

function applyPayoutRuntimeConfig(root) {
    const configNode = (root || document).querySelector('#payout-runtime-config');
    if (!configNode) {
        return;
    }

    dailyWageRate = Number(configNode.dataset.dailyWageRate || 0);
    payoutDays = Number(configNode.dataset.payoutDays || 0);
    beneficiaryPayoutRate = Number(configNode.dataset.beneficiaryPayoutRate || 0);
}

let payoutPartialRefreshInFlight = false;

async function refreshPayoutView() {
    if (payoutPartialRefreshInFlight) {
        return;
    }

    payoutPartialRefreshInFlight = true;
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('Unable to refresh payout view.');
        }

        const html = await response.text();
        const doc = new DOMParser().parseFromString(html, 'text/html');
        const nextBody = doc.querySelector('#payout-card-body');
        const currentBody = document.querySelector('#payout-card-body');

        if (!nextBody || !currentBody) {
            throw new Error('Unable to locate payout content.');
        }

        currentBody.innerHTML = nextBody.innerHTML;
        applyPayoutRuntimeConfig(currentBody);
        initializePayoutTable();
    } finally {
        payoutPartialRefreshInFlight = false;
    }
}

$(document).ready(function () {
    applyPayoutRuntimeConfig(document);
    initializePayoutTable();
});
...
function showEditForm(group) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Swal.fire('Success!', 'Municipality payout records updated successfully.', 'success').then(() => {
                refreshPayoutView().catch(() => window.location.reload());
            });
        }
    });
}
