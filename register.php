...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $positionMap = kodus_position_map();
    ...
    if (
      ...
      empty($position) || empty($positionAbr) || empty($area)
        ...
        notification_finish_response("<script>
          Swal.fire({
            title: 'Error',
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        notification_finish_response('<script>Swal.fire("Error", "Invalid email format!", "error").then(() => { history.back(); });</script>');
        exit;
    }
    ...
    if ($stmt->execute()) {
      ...
      password_policy_mark_compliant($conn, $userId);

      // Log audit
      $ip = $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN';
      $details = "$first_name $last_name ($username) registered.";
      audit_log($conn, (int) $userId, 'Register', $details, $ip);
      app_notification_create($conn, [
          'category' => 'users',
          'title' => 'New user registration',
          'message' => trim($first_name . ' ' . $last_name) . " registered a new KODUS account ({$username}).",
      ...
      $mailBody = notification_render_email_shell(
          ...
          . notification_render_action_button($app_root ?? './', 'Open KODUS', '#198754'),
          '#198754'
      );
      ...
      notification_finish_response('<script>Swal.fire("Error", "Registration failed!", "error");</script>');
  }
