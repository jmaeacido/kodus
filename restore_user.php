...
$stmt->execute();
$currentUser = db_stmt_fetch_one_assoc($stmt);
...
// Get user's info
$userStmt = $conn->prepare("SELECT email, first_name, last_name, deleted_at FROM users WHERE id = ?");
$userStmt->bind_param("i", $userIdToRestore);
$userStmt->execute();
$user = db_stmt_fetch_one_assoc($userStmt);
...
if ($restore->execute()) {
    role_change_clear($conn, $userIdToRestore);

    // Audit log
    $action = 'Restore Account';
    $details = 'Updated user ID ' . $userIdToRestore . ' | Changes: ' . audit_format_field_changes(
    ...
    try {
        ...
        $mail->Body = notification_render_email_shell(
            ...
            . '<p>Your account has been successfully restored by an administrator.</p>'
    ...
    security_send_json([
        'success' => true,
        'message' => $emailError
            ? 'User has been restored, but the notification email could not be sent.'
            : 'User has been restored and notified.',
